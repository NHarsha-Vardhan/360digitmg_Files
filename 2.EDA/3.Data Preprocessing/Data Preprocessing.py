...
import pandas as pd
import numpy as np
data = pd.read_excel(r"F:\Liser Time\360digitmg\4.PowerBI\data sets for assignment\Sample - Superstore.xlsm")
data.dtypes
...
import pandas as pd
data = pd.read_csv(r"F:\Liser Time\360digitmg\2.EDA\Data Sets\InClass_DataPreprocessing_datasets\mtcars_dup.csv")
...
cars = pd.read_excel(r"F:\Liser Time\360digitmg\4.PowerBI\data sets for assignment\Sample - Superstore.xlsm")
...
df = pd.read_csv(r"F:\Liser Time\360digitmg\2.EDA\Data Sets\InClass_DataPreprocessing_datasets\ethnic diversity.csv")
df.dtypes
df.describe()
df.Salaries.median()
...
IQR = data['Salaries'].quantile(0.75) - data['Salaries'].quantile(0.25)

lower_limit = data['Salaries'].quantile(0.25) - (IQR * 1.5)
upper_limit = data['Salaries'].quantile(0.75) + (IQR * 1.5)

############### 1. Remove (let's trim the dataset) ################
# Trimming Technique
# Let's flag the outliers in the dataset

outliers_df = np.where(data.Salaries > upper_limit, True, np.where(data.Salaries < lower_limit, True, False))
...
import pandas as pd
data = pd.read_csv(r"F:\Liser Time\360digitmg\2.EDA\Data Sets\InClass_DataPreprocessing_datasets\ethnic diversity.csv")
data.head()
data.tail()
...
df_new_1 = pd.get_dummies(df, drop_first = True) #  It's common to use drop_first=True  when dealing with categorical variables with a high cardinality to avoid multicollinearity in models like linear regression. However, the choice depends on your specific analysis and the requirements of your model.
...
df = pd.read_csv(r"F:\Liser Time\360digitmg\2.EDA\Data Sets\InClass_DataPreprocessing_datasets\ethnic diversity.csv") # for doing modifications
...
education = pd.read_csv(r"F:\Liser Time\360digitmg\2.EDA\Data Sets\education.csv")

import scipy.stats as stats
import pylab

# Checking whether data is normally distributed
a=stats.probplot(education.gmat, dist = "norm", plot = pylab)
...
### Normalization
## load dataset
ethnic1 = pd.read_csv(r"F:\Liser Time\360digitmg\2.EDA\Data Sets\InClass_DataPreprocessing_datasets\ethnic diversity.csv")
ethnic1.columns
